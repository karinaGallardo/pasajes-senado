...
func (r *SolicitudRepository) FindWithOpenTicketDescargoPaginated(ctx context.Context, userID string, isAdmin bool, page, limit int, searchTerm string) (*PaginatedSolicitudes, error) {
	var solicitudes []models.Solicitud
	var total int64
...
func (r *SolicitudRepository) CountWithOpenTicketDescargo(ctx context.Context, userID string, isAdmin bool) (int64, error) {
	var count int64
	baseQuery := r.db.WithContext(ctx).Model(&models.Solicitud{}).
		Joins("JOIN descargos ON solicitudes.id = descargos.solicitud_id").
		Where("descargos.estado = ?", models.EstadoDescargoOpenTicket)
