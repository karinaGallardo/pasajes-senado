...
func (s *ReportService) GeneratePV02(ctx context.Context, solicitud *models.Solicitud, personaView *models.MongoPersonaView) *gofpdf.Fpdf {
	...
	pdf.SetXY(25, 250)
	pdf.CellFormat(70, 4, tr("FIRMA / SELLO UNIDAD SOLICITANTE"), "", 1, "C", false, 0, "")
...
func (s *ReportService) GeneratePV05(ctx context.Context, descargo *models.Descargo, personaView *models.MongoPersonaView) *gofpdf.Fpdf {
	...
	esOficial := solicitud.CupoDerechoItemID == nil || *solicitud.CupoDerechoItemID == ""
	...
	subtitle := tr("PASAJES AEREOS PARA SENADORAS Y SENADORES")
	if esOficial {
		subtitle = tr("PASAJES, VIÁTICOS Y GASTOS DE REPRESENTACIÓN")
	}
	pdf.SetXY(xHeader+40, yHeader+11)
	pdf.SetFont("Arial", "B", 9)
	pdf.CellFormat(115, 6, subtitle, "", 1, "C", false, 0, "")

	line3 := fmt.Sprintf("GESTION: %s", solicitud.CreatedAt.Format("2006"))
	if esOficial {
		line3 = tr("SERVIDORES PÚBLICOS") + " - " + line3
	}
	pdf.SetXY(xHeader+40, yHeader+16)
	pdf.SetFont("Arial", "B", 10)
	pdf.CellFormat(115, 6, line3, "", 1, "C", false, 0, "")
	...
	if esOficial {
		...
		drawLabelBox(tr("UNIDAD FUNCIONAL :"), unidadStr, 40, 120, false)
	} else {
		origenUser := ""
		if solicitud.Usuario.Origen != nil {
			origenUser = solicitud.Usuario.Origen.Ciudad
	...
	// Sección viáticos y gastos de representación (solo solicitud oficial)
	if esOficial && len(solicitud.Viaticos) > 0 {
		pdf.SetFont("Arial", "B", 10)
	...
	// Devolucion
	pdf.SetFont("Arial", "B", 9)
	devolucionLabel := tr(" DEVOLUCIÓN DE PASAJES POR DERECHO")
	if esOficial {
		devolucionLabel = tr(" DEVOLUCIÓN DE PASAJES (si aplica)")
	}
	pdf.CellFormat(190, 6, devolucionLabel, "B", 1, "L", false, 0, "")
	pdf.SetFont("Arial", "", 8)
	...
	firmaLabel := tr("SENADORA / SENADOR")
	if esOficial {
		firmaLabel = tr("SERVIDOR(A) PÚBLICO(A)")
	}
	// Left (Senador / Servidor)
	pdf.Line(35, sigY+10, 95, sigY+10)
	pdf.SetXY(35, sigY+12)
	pdf.SetFont("Arial", "B", 7)
	pdf.CellFormat(60, 4, firmaLabel, "", 1, "C", false, 0, "")
	pdf.SetX(35)
	pdf.SetFont("Arial", "", 7)
	pdf.CellFormat(60, 4, tr(solicitud.Usuario.GetNombreCompleto()), "", 1, "C", false, 0, "")
	pdf.SetX(35)
